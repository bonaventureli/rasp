/****************************************************************************
** Meta object code from reading C++ file 'DrawingContext.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.2.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "b9edit/DrawingContext.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'DrawingContext.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.2.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_DrawingContext_t {
    QByteArrayData data[8];
    char stringdata[91];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_DrawingContext_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_DrawingContext_t qt_meta_stringdata_DrawingContext = {
    {
QT_MOC_LITERAL(0, 0, 14),
QT_MOC_LITERAL(1, 15, 11),
QT_MOC_LITERAL(2, 27, 0),
QT_MOC_LITERAL(3, 28, 7),
QT_MOC_LITERAL(4, 36, 3),
QT_MOC_LITERAL(5, 40, 11),
QT_MOC_LITERAL(6, 52, 18),
QT_MOC_LITERAL(7, 71, 18)
    },
    "DrawingContext\0SetUpperImg\0\0QImage*\0"
    "img\0SetLowerImg\0GenerateLogicImage\0"
    "GenerateGreenImage\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_DrawingContext[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    1,   34,    2, 0x0a,
       5,    1,   37,    2, 0x0a,
       6,    0,   40,    2, 0x0a,
       7,    0,   41,    2, 0x0a,

 // slots: parameters
    QMetaType::Void, 0x80000000 | 3,    4,
    QMetaType::Void, 0x80000000 | 3,    4,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void DrawingContext::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        DrawingContext *_t = static_cast<DrawingContext *>(_o);
        switch (_id) {
        case 0: _t->SetUpperImg((*reinterpret_cast< QImage*(*)>(_a[1]))); break;
        case 1: _t->SetLowerImg((*reinterpret_cast< QImage*(*)>(_a[1]))); break;
        case 2: _t->GenerateLogicImage(); break;
        case 3: _t->GenerateGreenImage(); break;
        default: ;
        }
    }
}

const QMetaObject DrawingContext::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_DrawingContext.data,
      qt_meta_data_DrawingContext,  qt_static_metacall, 0, 0}
};


const QMetaObject *DrawingContext::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *DrawingContext::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_DrawingContext.stringdata))
        return static_cast<void*>(const_cast< DrawingContext*>(this));
    return QWidget::qt_metacast(_clname);
}

int DrawingContext::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
